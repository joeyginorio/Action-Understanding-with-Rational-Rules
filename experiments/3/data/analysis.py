...
lesionModel = modelResults.loc[modelResults.Model=='Full Lesion',:]
...
sns.regplot(y,x,color='blue',scatter_kws=dict(color='black'),ax=ax_1)
plt.ylim([-2,3])
...
# plt.show()
plt.savefig('models.pdf',dpi=200)
plt.close()
...
for i in range(20):
	...
	# plt.plot(x, turk[i],'co')
	plt.errorbar(x, turk[i],yerr=[abs(turk[i]-lowCIs[i]),abs(turk[i]-highCIs[i])],fmt='co',linewidth=1.5,barsabove=True,elinewidth=2,capsize=10,capthick=2)
...
legend = fig.legend((f[0],t[0],l[0]),('Model Prediction','Participant judgments','Lesion Prediction'),fontsize=12,loc=(.78,.01),shadow=True,fancybox=True)
legend.get_frame().set_facecolor('white')
# plt.tight_layout()
plt.savefig('trials.pdf',dpi=200)
...
temp0 = [i[i.Trial==20].Result.values[0] for i in participants]
temp1 = [i[i.Trial==20].Result.values[1] for i in participants]
temp2 = [i[i.Trial==20].Result.values[2] for i in participants]
ci0 = bootstrap_mean(temp0)
ci1 = bootstrap_mean(temp1)
ci2 = bootstrap_mean(temp2)

lo = [turk[19][0]-ci1[0], turk[19][1]-ci2[0], turk[19][2]-ci0[0]]
hi = [ci1[1]-turk[19][0], ci2[1]-turk[19][1], ci0[1]-turk[19][2]]
ax.grid(linestyle='-',linewidth=.20,color='gray')

sns.barplot(range(3),turk[19],ax=ax, yerr=(lo,hi),ecolor='black',error_kw=dict(elinewidth=2,capsize=10,capthick=2),edgecolor='black',linewidth=1)
ax.set_title('Participants',fontsize=16)
plt.ylim([-1.5,2.5])
plt.xticks(x,fullNames[19],rotation=-20,horizontalalignment='center',fontsize=15)
...
sns.barplot(range(3),full[19],ax=ax,edgecolor='black',linewidth=1)
...
sns.barplot(range(3),lesion[19],ax=ax,edgecolor='black',linewidth=1)
...
fig5.text(0.06,0.7,'Judgment',rotation='90',fontsize=21)
fig5.text(0.5,.06,'Hypothesis',fontsize=21)

plt.savefig('fig5.pdf',dpi=200)
...
fig6.text(0.06,0.7,'Judgment',rotation='90',fontsize=21)
fig6.text(0.5,.06,'Hypothesis',fontsize=21)
...
# lo = [turk[21][0]-ci2[0], turk[21][1]-ci1[0], turk[21][2]-ci0[0]]
# hi = [ci2[1]-turk[21][0], ci1[1]-turk[21][1], ci0[1]-turk[21][2]]
# sns.barplot(range(3),turk[21],ax=ax, yerr=(lo,hi),ecolor='black',error_kw=dict(elinewidth=2,capsize=10,capthick=2),edgecolor='black',linewidth=1)
# ax.set_title('Participants')
# plt.ylim([-1.5,1.5])
# plt.xticks(x,fullNames[21],rotation=-20,horizontalalignment='center',fontsize=12)
# # plt.errorbar(range(3),yerr=[[-1,-1,-1],[1,1,1]])
...
# # plt.margins(.2)
# # ax.set_title('Full Model')
# sns.barplot(range(3),full[21],ax=ax,edgecolor='black',linewidth=1)
# plt.ylim([-1.5,1.5])
...
# # plt.ylim([-1.5,1.5])
# # ax.set_title('Prior only')
# sns.barplot(range(3),lesion[21],ax=ax,edgecolor='black',linewidth=1)
