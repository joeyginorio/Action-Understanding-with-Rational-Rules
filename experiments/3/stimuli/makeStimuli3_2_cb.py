...
class makeStimuli():
	...
	def __init__(self):
		...
		self.coords = [
		{'2':(-200,300), '1':(-800,-300),'S':(200,300),'3':(800,-300),'4':(0,-100),
		...
		for k in range(len(self.coords)):
			...
			for i in range(1):
				...
				for j in range(len(self.allPaths)):
					...
					turtle.update()
					ts = turtle.getscreen()
					ts.getcanvas().postscript(file="batch2_cb/stim"+str(k)+"_"+"set"+str(i)+"_"+"path"+str(j)+".eps")
					self.wn.clear()
					turtle.tracer(0,0)
	...
	def drawStimulus(self, wn, path, coords):
		...
		for i in range(len(path)):
			...
			if i==len(path)-1:
				...
				nextSpot = path[i+1]
			if i > 0 and ((path[i-1]=='1' and path[i]=='3' and path[0] == '6') or
			(path[i-1]=='2' and path[i]=='5' and path[0] == '6') ):
				t = True
