...
class makeStimuli():
	...
	def __init__(self):
		...
		self.coords = [
		{'S':(800,300), '1':(-800,300),'2':(-200,-300),'3':(800,-300),'4':(0,100),
		'5':(-200,-100),'6':(-200,-100), '7':(400,100), '8':(600,300)},
		]
		...
		for k in range(len(self.coords)):
			...
			for i in range(1):
				...
				for j in range(len(self.allPaths)):
					...
					turtle.update()
					ts = turtle.getscreen()
					ts.getcanvas().postscript(file="batchtut/stim_2_"+str(k)+"_"+"set"+str(i)+"_"+"path"+str(j)+".eps")
					self.wn.clear()
					turtle.tracer(0,0)
