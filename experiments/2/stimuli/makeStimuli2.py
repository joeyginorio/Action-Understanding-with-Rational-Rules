...
class makeStimuli():
	...
	def __init__(self):
		...
		self.allPaths = list()
		for i in range(1,5):
			self.allPaths += list(itertools.permutations('1234',i))
	...
	def drawStimulus(self, wn, path, coords):
		...
		for i in range(len(path)):
			...
			if i > 0 and ((path[i-1]=='3' and path[i]=='1' and path[0] != '2') ):
				t = True
				coords = self.changeCoords(coords,-40)
	...
	def drawPath(self, spot, coords,last,t):
		if t:
			self.agentTracer.up()
			self.agentTracer.goto(self.agentTracer.position()[0],self.agentTracer.position()[1]-40)
